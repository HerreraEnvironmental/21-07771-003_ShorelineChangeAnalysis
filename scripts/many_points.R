...
ggplot(data = complete.profile %>% filter(year == year.pattern)) +
  ...
  geom_point(aes(x = xmin, y = ymin), color = "red", size = 3) +
  ...
  geom_point(aes(x = xmax, y =  ymax), color = "purple", size = 3) +
...
change <- complete.profile %>% 
  ...
  unique() %>%
  group_by(profile) %>% 
  mutate(xmin_rate = 100 * (xmin - lag(xmin))/lag(xmin)) %>%
  mutate(ymin_rate = 100 * (ymin - lag(ymin))/lag(ymin)) %>%
  mutate(min_rate = xmin_rate/ymin_rate)
