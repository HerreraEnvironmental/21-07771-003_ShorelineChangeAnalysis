...
Increasing Euclidean distance = Accretion Decreasing Euclidean distance = Erosion
...
*TODO check this*
The regression t-Test is applied to test if the slope, β1
, of the population regression line equals 0
. Based on that test we may decide whether x
...
## Comparison of results to Washington Coastal Erosion Hazard Assessment 
```{r}
...
This obscures the variation inherent in the movement of profiles over time.

Each profile, while following the same basic layout, comes with the variation expected from environmental data.
Therefore it is important to find a method of individual profile characterization that will apply to all 4500 profiles, and can act as a reliable proxy to determine erosion or accretion.
...
These quartiles are labeled "minimum", "quartile1", "median", "quartile3", and "maximum" in accordance with standard boxplot parameters.
...
How do those points change with time across all profiles?
