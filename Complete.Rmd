...
```{r single_profile_visualization, include=TRUE, echo=FALSE}
...
```{r complete_profile_visualization, echo=FALSE, include=TRUE}
profile.pattern <- "prof_17"
...
```{r complete_profile_timeseries, echo=FALSE, include=TRUE}
...
```{r profiles_BasePoint, echo=FALSE, include=TRUE}
...
```{r midpoint_euclidean, echo=FALSE, include=TRUE}
...
```{r single_linear_quartiles, echo=FALSE, include=TRUE}
...
```{r rates_of_change, echo=FALSE, include=TRUE}
profile.pattern <- "prof"
...
This analysis applies unsupervised clustering, as the input is unlabeled data without training.
Relevant types of unsupervised clustering are k-means clustering and hierarchical agglomerative clustering (HCA).

Pure kmeans requires some advance knowledge of the number of clusters, the "k" variable, while HCA seeks to build a hierarchy of clusters without a predetermined k.
